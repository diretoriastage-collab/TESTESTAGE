...
function normalizarBoolean(valor, padrao = true) {
    ...
    return !['false', '0', 'nao', 'não', 'inativo', 'encerrada'].includes(str);
}

function asNumber(valor, padrao = 0) {
...
function ensureStageBadgeStyles() {
    ...
    style.textContent = `
        @keyframes stage-fire-ring { 0%{transform:rotate(0deg) scale(1);}50%{transform:rotate(180deg) scale(1.08);}100%{transform:rotate(360deg) scale(1);} }
        @keyframes stage-new-core { 0%,100%{transform:scale(1);box-shadow:0 0 16px rgba(255,209,102,0.9);}50%{transform:scale(1.08);box-shadow:0 0 28px rgba(255,71,87,1);} }
        .stage-new-badge { position:relative; display:inline-flex; align-items:center; justify-content:center; width:46px; height:46px; margin-left:10px; border-radius:50%; background:#130502; color:#fff; font-weight:900; font-size:10px; letter-spacing:.7px; text-transform:uppercase; vertical-align:middle; isolation:isolate; animation:stage-new-core 1.15s ease-in-out infinite; }
        .stage-new-badge::before { content:''; position:absolute; inset:-7px; border-radius:50%; background:conic-gradient(from 0deg,#ffea00,#ff8a00,#ff2f1f,#ff0066,#ff8a00,#ffea00); z-index:-2; animation:stage-fire-ring 1.1s linear infinite; filter:drop-shadow(0 0 10px rgba(255,100,0,.9)); }
        .stage-new-badge::after { content:''; position:absolute; inset:-2px; border-radius:50%; background:radial-gradient(circle,#ff7133 0%,#b31312 54%,#2a0502 72%); z-index:-1; }
        @keyframes stage-bonus-pulse { 0%,100%{transform:scale(1);box-shadow:0 0 22px rgba(255,80,16,0.7);}50%{transform:scale(1.08);box-shadow:0 0 42px rgba(255,180,10,1);} }
        @keyframes stage-bonus-ring { from{transform:rotate(0deg);}to{transform:rotate(360deg);} }
        .stage-bonus-widget { position:fixed; bottom:24px; right:24px; width:96px; height:96px; border-radius:50%; background:#180604; color:#fff; display:flex; align-items:center; justify-content:center; text-align:center; line-height:1.05; font-size:12px; font-weight:900; box-shadow:0 0 34px rgba(255,90,10,0.85); cursor:pointer; z-index:9999; animation:stage-bonus-pulse 1.45s ease-in-out infinite; isolation:isolate; border:none; }
        .stage-bonus-widget::before { content:''; position:absolute; inset:-9px; border-radius:50%; background:conic-gradient(#ffeb3b,#ff9800,#ff2d1f,#ff007a,#ff9800,#ffeb3b); z-index:-2; animation:stage-bonus-ring 1.4s linear infinite; }
        .stage-bonus-widget::after { content:''; position:absolute; inset:0; border-radius:50%; background:radial-gradient(circle at 35% 25%,#ffe066 0%,#ff6b35 42%,#d91e18 68%,#520604 100%); z-index:-1; }
        .stage-bonus-widget:hover { transform:scale(1.06); }
        ...
        .stage-bonus-modal { width:min(520px, calc(100vw - 34px)); aspect-ratio:1/1; max-height:calc(100vh - 34px); border-radius:50%; background:radial-gradient(circle at 45% 20%,#3b1111 0%,#170707 58%,#070303 100%); padding:42px; box-shadow:0 0 70px rgba(255,90,10,0.35),0 0 0 10px rgba(255,122,24,.16); color:#fff; text-align:center; position:relative; display:flex; flex-direction:column; justify-content:center; overflow:hidden; }
        .stage-bonus-modal::before { content:''; position:absolute; inset:10px; border-radius:50%; border:2px dashed rgba(255,197,80,.35); animation:stage-bonus-ring 8s linear infinite; pointer-events:none; }
        .stage-bonus-modal h2 { margin:0 0 12px; font-size:28px; }
        .stage-bonus-modal p { margin:10px 0; font-size:15px; color:#ddd; }
        .stage-bonus-modal .stage-bonus-list { max-height:210px; overflow:auto; padding-right:4px; position:relative; z-index:1; }
        .stage-bonus-modal .stage-prize { margin:12px auto 0; padding:14px 18px; border-radius:26px; background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.14); font-size:14px; color:#fff; font-weight:700; }
        .stage-bonus-modal .stage-prize strong { display:block; margin-top:6px; font-size:18px; color:#ffd166; }
        ...
        @media (max-width:520px) { .stage-bonus-modal { border-radius:34px; aspect-ratio:auto; min-height:420px; padding:30px 22px; } .stage-bonus-widget { width:82px; height:82px; font-size:11px; right:16px; bottom:16px; } }
    `;
...
function isPromocaoAtiva(promocao) {
    ...
    return normalizarBoolean(promocao.ativa, true) && !promocao.concluida && !isNaN(inicio) && !isNaN(fim) && agora >= inicio && agora <= fim;
}
...
function renderBonusAtivoWidget() {
    ...
    widget.innerHTML = 'BONUS<br>ATIVO';
}
...
function mostrarModalBonusAtivo() {
    ...
    `;
    const bonusCardsHtml = ativa.map(p => `
        <div class="stage-prize">
            ${String(p.tipo || 'Meta').toUpperCase()} - ${p.quantidade || 0} ponto(s)<br>
            <small>${new Date(p.inicio).toLocaleString('pt-BR')} ate ${new Date(p.fim).toLocaleString('pt-BR')}</small>
            <strong>${p.premio || 'Premio surpresa'}</strong>
        </div>
    `).join('');
    overlay.innerHTML = `
        <div class="stage-bonus-modal">
            <h2>BONUS ATIVO</h2>
            <p>${ativa.length} promocao${ativa.length > 1 ? 'es' : ''} valendo agora.</p>
            <div class="stage-bonus-list">${bonusCardsHtml}</div>
            <button class="stage-close-btn" onclick="fecharModalBonusAtivo()">Bora vender</button>
        </div>
    `;
...
async function sincronizarMetasVendas() {
  try {
    ...
    if (resp && resp.metas) {
      DB.metas.diariaVendas = asNumber(resp.metas.diariaVendas ?? resp.metas.diaria ?? resp.metas.Diaria, 10);
      DB.metas.quinzenalVendas = asNumber(resp.metas.quinzenalVendas ?? resp.metas.quinzenal ?? resp.metas.Quinzenal, 75);
      DB.metas.mensalVendas = asNumber(resp.metas.mensalVendas ?? resp.metas.mensal ?? resp.metas.Mensal, 150);
      salvarDB();
    }
...
async function sincronizarMetasInstalacoes() {
  try {
    ...
    if (resp && resp.metas) {
      DB.metas.instalacoes = resp.metas.map(m => ({
        id: m.id,
        tipo: m.tipo || m.Tipo || 'empresa',
        entidade: m.entidade || m.Entidade || 'STAGE TELECOM',
        entidadeId: asNumber(m.entidadeId ?? m.EntidadeId, 0),
        diaria: asNumber(m.diaria ?? m.Diaria, 0),
        quinzenal: asNumber(m.quinzenal ?? m.Quinzenal, 0),
        mensal: asNumber(m.mensal ?? m.Mensal, 0)
      }));
      salvarDB();
    }
...
async function sincronizarPromocoes() {
  try {
    ...
    if (resp && resp.promocoes) {
      DB.promocoes = resp.promocoes.map(p => ({
        id: p.id ?? p.ID ?? p.Id ?? Date.now() + Math.random(),
        tipo: p.tipo || p.Tipo || 'vendas',
        quantidade: asNumber(p.quantidade ?? p.Quantidade, 0),
        inicio: p.inicio || p.Inicio || p['Início'] || '',
        fim: p.fim || p.Fim || '',
        premio: p.premio || p.Premio || p['Prêmio'] || '',
        ativa: normalizarBoolean(p.ativa ?? p.Ativa ?? p.Status, true),
        concluida: normalizarBoolean(p.concluida ?? p.Concluida, false),
        vencedores: Array.isArray(p.vencedores) ? p.vencedores : []
      }));
      salvarDB();
    ...
    try {
        ...
        if (resp && resp.vendas && Array.isArray(resp.vendas)) {
            const aprovadasNuvem = resp.vendas.map(v => {
                const idVenda = v.UUID || (v.Cliente + Date.now());
                const original = DB.ativacoes.find(old => idsIguais(old.id, idVenda));
                return {
                id: idVenda,
                ...
                status: 'Aprovado',
                vendedorNome: v.Vendedor || original?.vendedorNome || '',
                vendedor_id: v.VendedorId ? parseInt(v.VendedorId) : null,   // 🔥 CORRIGIDO
                data: v['Data Aprovação'] ? formatarBR(v['Data Aprovação']) : hojeBR(),
                vendedor_id: v.VendedorId ? parseInt(v.VendedorId) : original?.vendedor_id || null,
                finalizada: true,
            ...
            if (sessao.tipo === 'admin' && document.getElementById('secao-dashboard')?.classList.contains('section-active')) renderDashboard();
...
function carregarInicioVendedor() {
    ...
    carregarPainelMetasVendedor(vendasAprovadas);
    renderBonusAtivoWidget();
}

function encontrarMetaInstalacaoVendedor() {
    const metas = DB.metas.instalacoes || [];
    return metas.find(m => m.tipo === 'vendedor' && idsIguais(m.entidadeId, sessao.id)) ||
        metas.find(m => m.tipo === 'empresa') ||
        null;
}

function carregarPainelMetasVendedor(vendasAprovadas = []) {
    const painel = document.getElementById('painelMetasVendedor');
    if (!painel || !sessao) return;
    const hoje = hojeBR();
    const vendasHoje = vendasAprovadas.filter(v => v.data === hoje).length;
    const vendasMes = vendasAprovadas.length;
    const metaInstalacao = encontrarMetaInstalacaoVendedor();
    const instaladosMes = vendasAprovadas.filter(v => v.instalacaoStatus === 'Instalado').length;
    const cards = [
        { label: 'Vendas hoje', value: `${vendasHoje}/${DB.metas.diariaVendas || 10}`, note: 'Meta diaria' },
        { label: 'Vendas mes', value: `${vendasMes}/${DB.metas.mensalVendas || 150}`, note: 'Meta mensal' }
    ];
    if (metaInstalacao) {
        cards.push({ label: 'Instalacoes mes', value: `${instaladosMes}/${metaInstalacao.mensal || 0}`, note: metaInstalacao.tipo === 'vendedor' ? 'Meta individual' : 'Meta empresa' });
    }
    (DB.metas.produtos || []).slice(0, 3).forEach(meta => {
        const totalProduto = vendasAprovadas.filter(v => (v.produto || v.plano) === meta.produto).length;
        cards.push({ label: meta.produto, value: `${totalProduto}/${meta.mensal || 0}`, note: 'Produto no mes' });
    });
    painel.innerHTML = cards.map(card => `
        <div class="meta-vendedor-card">
            <span class="meta-vendedor-label">${card.label}</span>
            <span class="meta-vendedor-value">${card.value}</span>
            <span class="meta-vendedor-note">${card.note}</span>
        </div>
    `).join('');
}
...
// ===== CONFIGURAÇÕES =====
const GOOGLE_SHEET_VENDAS_URL = 'https://script.google.com/macros/s/AKfycbzNd32I0yV9G202eaq4M_GKYyWRim-eWDpwZw1M33Ck8Cz8JtwH1wDomTRTiqzfdZiq/exec'; // SUBSTITUA PELO SEU NOVO URL
